...
export function FormField({ label, name, fieldType = 'input', inputType = 'text', placeholder = '' }: FormFieldProps) {
	const styles =
	...
	return (
		<>
			...
			{fieldType === 'input' ? (
				<input id={name} type={inputType} name={name} placeholder={placeholder} className={`${styles} h-10`} required />
			) : (
